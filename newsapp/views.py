...
def index(request):
    ...
    if search:
        content_list_all = Content.objects.filter(title__iregex="\w*%s\w*" % search)
    elif type_id:
        # 过滤出不同新闻类型的数据
        type_id = int(type_id)
...
class News(View):
    def get(self, request, content_id):
        ...
        form = CommentForm()
        form = CommentForm()
        comment_list = Comment.objects.filter(news_id=contents, restore=None)
